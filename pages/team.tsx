...
export interface AuthorsPageProps { authors: Author[], associates: Associate[] }
const AuthorsPage: NextPage<AuthorsPageProps> = ({ authors, associates }) => {
	return (
		<div className="bg-slate-50">
      <Navbar />
      <div className="relative px-4 px-6 py-8 lg:px-6 lg:py-20">
        <h1 className="block mx-auto text-base text-4xl font-extrabold leading-8 tracking-wide text-center text-gray-900 uppercase">
          Core Team
        </h1>
      </div>
      <ul className="container grid grid-cols-1 gap-10 px-6 pb-8 mx-auto md:grid-cols-2 xl:grid-cols-4">
        {authors.map((author) => (
          <li
            key={author.name}
            className="relative overflow-hidden bg-white border border-gray-300 rounded shadow-sm"
          >
            <div>
              ...
              <footer className="absolute inset-x-0 bottom-0 flex items-center justify-center gap-4 p-4">
                {author.social.linkedIn ? (
                  <Link href={author.social.linkedIn}>
                    <a aria-label="LinkedIn" target="_blank" rel="noreferrer">
                      <svg
                        className="w-6 h-6 fill-current text-slate-700 dark:text-slate-200 hover:text-slate-600 dark:hover:text-slate-400"
                ...
                {author.social.github ? (
                  <Link href={author.social.github}>
                    <a aria-label="LinkedIn" target="_blank" rel="noreferrer">
                      <svg
                        className="w-6 h-6 fill-current text-slate-700 hover:text-slate-600 "
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 512 512"
                      >
      ...
      <div className="relative px-4 px-6 py-8 lg:px-8 lg:py-20">
        <h1 className="block mx-auto mb-16 text-base text-4xl font-extrabold leading-8 tracking-wide text-center text-gray-900 uppercase ">
          Associates
        </h1>
        <div className="container grid grid-cols-1 gap-10 px-6 pb-8 mx-auto md:grid-cols-2 xl:grid-cols-4">
          {associates.map((associate) => (
            <div
              key={associate.name}
              className="relative flex items-center p-6 space-x-3 bg-white border border-gray-300 rounded shadow-sm "
            >
              ...
              <div className="flex items-center gap-2">
                {associate.social?.linkedIn ? (
                  <Link href={associate.social?.linkedIn}>
                    <a aria-label="LinkedIn" target="_blank" rel="noreferrer">
                      <svg
                        className="w-6 h-6 fill-current text-slate-700 dark:text-slate-200 hover:text-slate-600 dark:hover:text-slate-400"
                        xmlns="http://www.w3.org/2000/svg"
                ...
                {associate.social?.github ? (
                  <Link href={associate.social?.github}>
                    <a aria-label="LinkedIn" target="_blank" rel="noreferrer">
                      <svg
                        className="w-6 h-6 fill-current text-slate-700 hover:text-slate-600 "
                        xmlns="http://www.w3.org/2000/svg"
			...
			associates: [
				{
					...
					role: "",
					social: {
						github: "https://github.com/chronark",
						linkedIn: "https://www.linkedin.com/in/andreas-thomas-1ba1751b6",
					},
				},
				{ name: "Seline Winkler" },
				{ name: "Lukas Greger" },
